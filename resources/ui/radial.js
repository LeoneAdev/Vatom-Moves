...
// Helper: Read a query parameter.
function getQueryParam(param) {
  const urlParams = new URLSearchParams(window.location.search);
  return urlParams.get(param);
}

// Define categories and their items in the specified order.
const categories = [
  {
    name: "Basic",
    items: [
...
// If a category is provided via query parameter, use it.
const catParam = getQueryParam("cat");
if (catParam !== null) {
  currentCategoryIndex = parseInt(catParam);
}
...
function renderMenu() {
  ...
  // Offset so that the first item is at 12 o'clock (–90° offset)
